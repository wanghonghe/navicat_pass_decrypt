...
class Frame(wx.Frame):
    def __init__(self):
        ...
        self.label3 = wx.StaticText(self.window,size=(124, 30),pos=(25, 100),label='密码字符串：',name='staticText3',style=2321)
        label3_font = wx.Font(14,74,90,400,False,'Microsoft YaHei UI',28)
        self.label3.SetFont(label3_font)
        self.textctrl1 = wx.TextCtrl(self.window,size=(533, 30),pos=(25, 130),value='',name='text',style=0)
        ...
        self.button1 = wx.Button(self.window,size=(90, 30),pos=(120, 170),label='执 行',name='button1')
        ...
        self.button2 = wx.Button(self.window,size=(90, 30),pos=(240, 170),label='清 除',name='button2')
        ...
        self.button3 = wx.Button(self.window,size=(126, 30),pos=(360, 170),label='帮 助',name='button3')
        ...
        self.text_ctrl1 = wx.TextCtrl(self.window,size=(540, 130),pos=(25, 230),style=wx.TE_READONLY | wx.TE_MULTILINE | wx.TE_CENTER | wx.TE_WORDWRAP|wx.NO_BORDER|wx.TE_MULTILINE|wx.TE_NO_VSCROLL)
        self.text_ctrl1.SetBackgroundColour(wx.Colour(240,240,240))
        self.text_ctrl1.Hide()
        self.text_ctrl = wx.TextCtrl(self.window, size=(540, 170), pos=(25, 210),style=wx.TE_MULTILINE | wx.TE_READONLY | wx.TE_WORDWRAP|wx.NO_BORDER|wx.TE_NO_VSCROLL)
    ...
    def button3_down(self,event):
        ...
        if self.text_ctrl.IsShown():
            ...
            text = '''一、注册表查找密码，加密版本为11：
1、打开注册表并找到HCU\\SOFTWARE\\PremiumSoft\\Navicat\\Servers
2、点击相应的服务器，在右边窗口中找到字符串键Pwd，双击此键值
3、在出现的窗口中，“数值数据”下方的框中即为使用11版本加密的密码。
二、从软件中导出密码，加密版本为12：
1、点击软件的“文件”——“导出连接”
2、选择导出的服务器并勾选下方的“到处密码”，导出.ncx连接文件
3、使用文本编辑器打开.ncx文件，点击服务器，双击右边窗口中pwd字段，弹出窗口中的“数值数据”框中的字符串即为使用12版本加密的密码。
